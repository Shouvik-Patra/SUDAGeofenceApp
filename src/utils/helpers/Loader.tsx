...
export default function Loader({ visible = false, text="Finding your location..." }) {
  const rippleAnim = useRef(new Animated.Value(0)).current;
  const fadeAnim = useRef(new Animated.Value(0)).current;
  const rotateAnim = useRef(new Animated.Value(0)).current;

  useEffect(() => {
     let rippleAnimation: Animated.CompositeAnimation | null = null;
      ...
      <View style={{ alignItems: 'center', justifyContent: 'center' }}>
        <Animated.View
          style={{
            ...
            height: 120,
            borderRadius: 60,
            backgroundColor: Colors.grey || '#ffffff',
